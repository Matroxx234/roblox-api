...
// 🔹 Fonction pour récupérer toutes les pages de Game Pass
async function getAllGamePasses(userId, creatorType) {
  ...
  while (true) {
    ...
    const newPasses = (data.data || []).map((item) => ({
      id: item.id,
      name: item.name,
      price: item.price || 0,
      assetType: "GamePass"
...
// 🌐 Route test
app.get("/", (_, res) => {
  res.send("✅ API Roblox GamePass (pagination activée)");
});
...
app.get("/api/passes/:userId", async (req, res) => {
  ...
  try {
    ...
    // Si "User" retourne rien, essayer "Group"
